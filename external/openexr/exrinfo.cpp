...
mxArray * getAttributesMap(const Header& header)
{
    ...
    for (Header::ConstIterator it = header.begin(); it != header.end(); ++it) {
        ...
        attributes.push_back(pair);
        // printf("*****TEST HERE value %s", mxArrayToString(attributes[0].value));
    }
    ...
    if (mexCallMATLAB(1, &mapHandle, 2, mapArgs, "containers.Map") != 0) {
...
{ 
    ...
    try {
        ...
        mxArray* cNames = getChannelNames(header.channels());
        ...
        mxSetField(bStruct, 0, "channels",   cNames);
