...
%    generate driving scenes. This example works with the PBRT-V3 docker
%    container (not V2).
%
% See Also:
...
destDir = sprintf(strcat('/media/zhenyi27/8474e4a5-2b1b-444a-a496-', ...
    '2b8aee99a4f6/downloads/%s'), thisName{1});
...
%}
%% Process them
rootPath = strcat('/media/zhenyi27/8474e4a5-2b1b-444a-a496-', ...
    '2b8aee99a4f6/downloads');
cd(rootPath)
sceneList = dir();
sceneList(1:2) = [];
% NumImgs = 0;
...
% for ii = 1
    ...
    sensorImagesDir = fullfile(rootPath, sceneList(ii).name, ...
        'sensorImages');
...
% classVisulization = fullfile(sceneFigureDir, ...
%    [sceneName, '_class_color.png']);
% imwrite(uint8(ClassColorMap), classVisulization);
% instancelabel = fullfile(sceneFigureDir, ...
%    [sceneName, '_instance_label.png']);
% imwrite(uint16(InstanceMap), instancelabel);
% instanceColor = fullfile(sceneFigureDir, ...
%    [sceneName, '_instance_color.png']);
