%% t_piMaterialChange
% DEPRECATED - Change the material properties in a V3 PBRT scene
%
% Description:
...
%
% Notes:
%    We increased the amount of docker memory and swap to 12G and 2G. Not
%    sure this was necessary.
%
%    We switched the skypmap from noon, which ius 98M, to cloudy, which is
%    7M. That should speed up something, and maybe cause fewer crashes.
%
% History:
%    XX/XX/18  ZL   SCIEN Team, 2018
...
scene = sceneSet(scene, 'name', ...
    sprintf('Glass (%d)', thisR.integrator.maxdepth.value));
ieAddObject(scene);
sceneWindow;

%% Change materials to glass
% For this scene, the BODY material is attached to ???? object. We need to
% parse the geometry file to make sure. This will happen, but has not yet.
