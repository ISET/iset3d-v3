...
% Description:
%    *****Warning*****
%        This tutorial may be outdated. I would suggest using it only as
...
for ss = 1:length(sphereSeparationsDegrees)
    ...
    sphereDiameterDegrees = sphereDiameterPixels * ...
        renderingFovDegrees(ss) / renderingFovPixels;
    sphereSeparationMeters = sphereDistanceMeters * ...
        tand(sphereSeparationDegrees);
    sphereRadiusMeters = tand(sphereDiameterDegrees / 2) * ...
        sphereDistanceMeters;
    ...
    if (abs(renderingFovMm(ss) - oiGet(oi, 'width', 'mm')) / ...
            renderingFovMm(ss) > 1e-6)
    ...
    % Look for max and min in an RG image, with this created as the
    % difference between two images at different wavelengths.
    edgeTossPixels = 20;
    photons = oiGet(oi, 'photons');
    wavelengths = oiGet(oi, 'wave');
    indexGreen = find(wavelengths == 500);
    indexRed = find(wavelengths == 700);
    photonsGreen = squeeze(photons(1+edgeTossPixels:end-edgeTossPixels, ...
        1+edgeTossPixels:end-edgeTossPixels, indexGreen));
    photonsRed = squeeze(photons(1+edgeTossPixels:end-edgeTossPixels, ...
        1+edgeTossPixels:end-edgeTossPixels, indexRed));
    photonsRG = photonsRed/max(photonsRed(:)) - ...
        photonsGreen/max(photonsGreen(:));

    % Find red peak location
    gauassianSigmaPixels = 3;
    gaussianSizePixels = 9;
    convKernal = fspecial('gaussian', ...
        [gaussianSizePixels gaussianSizePixels], gauassianSigmaPixels);
