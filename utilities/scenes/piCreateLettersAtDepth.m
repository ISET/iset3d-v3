...
% Create a scene with letters at varying depths.
%
% Syntax:
%   recipe = piCreateLettersAtDepth([varargin])
%
% Description:
%    Create a recipe for a scene that consists of three letters (A, B, C)
%    placed at different distances away from the camera. The backdrop
%    consists of a checkerboard wall and ground. The letters area always
%    placed +/- 10 degrees away from the optical axis.
%
% Inputs:
%    None.
%
...
%    Adist        - Numeric. The distance from the camera to the letter A
%                   in meters. Default is 0.1.
%    Bdist        - Numeric. The distance from the camera to the letter B
%                   in meters. Default is 0.2.
%    Cdist        - Numeric. The distance from the camera to the letter C
%                   in meters. Default is 0.3.
%    illumination - String. The illumination of the scene (infinite light)
%                   as as SPD filename. Default is 'EqualEnergy.spd'.
%    nChecks      - Matrix. A 1x2 Matrix containing the number of checks in
%                   the pattern along the wall & ground respectively.
%                   Default is [64 64].
%    Adeg         - Numeric. The degrees of distance from the middle for
%                   the letter A. Default is 6.
%    Cdeg         - Numeric. The degrees of distance from the middle for
%                   the letter C. Default is 4.
%
...
if length(nchecks) == 1, nchecks(2) = nchecks(1); end
recipe = piMaterialTextureAdd(recipe, 'WallMaterial', 'checkerboard', ...
    'uscale', nchecks(1), 'vscale', nchecks(1), 'color1', [0.3 0.3 0.3]);
recipe = piMaterialTextureAdd(recipe, 'GroundMaterial', 'checkerboard', ...
    'uscale', nchecks(2), 'vscale', nchecks(2), 'color1', [0.3 0.3 0.3]);
