...
%
% Inputs:
%    renderRecipe - Object. An iset3d recipe object describing the
%                   rendering parameters. This includes the inputFile and
%                   the outputFile, which are used to find the  directories
%                   containing all of the pbrt scene data.
%
% Outputs:
%    renderRecipe - Object. The recipe object, updated by the processing
...
% Read all the text in the file, ignoring indents.
...
if ~convertedflag
    ...
    for dd = 1:length(nestbegin)
        ...
        for ii = nestbegin(dd):nestend(dd)
            ...
            if piContains(txtLines{nestbegin(dd) + 1}, '#ObjectName ')
                GroupObj_name_tmp = ...
                    erase(txtLines{nestbegin(dd) + 1}, '#ObjectName ');
                ...
                if piContains(txtLines{nestbegin(dd)+2}, 'ConcatTransform')
                    ...
                    tmp = textscan(tmp, strcat('%s [%f %f %f %f %f ', ...
            ...
            if piContains(txtLines(ii), 'Shape')
                ...
                elseif piContains(txtLines(ii - 4), ':Vector(')
                    ...
                    name = erase(txtLines(obj(jj - 1).index - 4), ...
                        '#ObjectName ');
                end
                name = name{1};
                index = strfind(name, ':');
                ...
                output_folder = fullfile(outFilepath, 'scene', ...
                    'PBRT', 'pbrt-geometry');
                outputGeometry = fullfile('scene', 'PBRT', ...
                    'pbrt-geometry', output_name);
                ...
                % Find 'integer indices', point P, normal N and put them in
                % their own geometry file. We write the data here.
    ...
    % future. The next time through, use the JSON file, which then passes
    % this function to the else condition.
    ...
    % There may be a utility that accomplishes this. We should find it and
    % use it here.
