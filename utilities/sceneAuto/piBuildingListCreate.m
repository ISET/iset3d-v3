...
% % Test now
% currentPath = pwd;
% BuildingFolder = strcat('/Volumes/group/data/NN_Camera_', ...
%     'Generalization/Pbrt_Assets_Generation/pbrt_assets/building');
% cd(BuildingFolder)
% kk = 1;
% for ii = 5:20
% fname = fullfile(BuildingFolder, sprintf('building_%03d', ii), ...
%     sprintf('building_%03d.json', ii));
...
%     buildingList(kk).geometryPath = fullfile(localFolder, 'scene', ...
%         'PBRT', 'pbrt-geometry');
%     kk = kk + 1;
% end
...
%}
function buildingList = piBuildingListCreate()
% Create the building list using data previously downloaded from Flywheel.
%
% Syntax:
%   buildingList = piBuildingListCreate()
%
% Description:
%    Create the building list using data that you have previously
%    downloaded from Flywheel.
%
% Inputs:
%    None.
%
% Outputs:
%    buildingList - Struct. The structure containing all of the buildings
%                   in the scene.
%
...
% Download from flywheel - Test now
currentPath = pwd;
BuildingFolder = strcat('/Volumes/group/data/NN_Camera_Generalization', ...
    '/Pbrt_Assets_Generation/pbrt_assets/building');
cd(BuildingFolder)

for ii = 1:24
fname = fullfile(BuildingFolder, sprintf('building_%03d', ii), ...
    sprintf('building_%03d.pbrt', ii));
    ...
    localFolder = fileparts(fname);
    buildingList(ii).geometryPath = fullfile(localFolder, 'scene', ...
        'PBRT', 'pbrt-geometry');
end
