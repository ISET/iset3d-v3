...
%
% Outputs:
%    thisCar     - Object. A recipe object containing all of the
%                  information relative to the car object in front of which
%                  the camera is placed.
%    from        - Vector. A 1x3 Vector of from coordinates.
%    to          - Vector. a 1x3 Vector of to coordinates.
%    orientation - String. A string describing the camera's orientation.
%                  Options include the following: 'front', 'left', 'right',
%                  and 'rear'.
%
...
%    04/18/19  JNM  Merge Master in (resolve conflicts). Additional doc.
...
if ~isempty(idx)
    ...
    idx = idx(idx_num);
    for jj = 1:length(carlist)
        if isempty(find((carlist(jj).pos == ...
                thisR.assets(idx{1}(1)).position(:, idx{1}(2))) == 0, 1))
            thisCar = carlist(jj);
            break;
        end
    end
