...
%                 'park', 'random' (random picks a random skymap from
%                 skymaps folder), and 'daytime' (daytime: 06:41-17:59).
%
% Outputs:
...
if ~piContains(skyName, ':')
    ...
    dataId = thisAcq.id;
    skyname = sprintf('probe_%02d-%02d_latlongmap.exr', ...
        str2double(time{1}), str2double(time{2}));
end
