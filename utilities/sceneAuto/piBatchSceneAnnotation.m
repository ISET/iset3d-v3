...
% Annotate the provided scene object.
%
% Syntax:
%   ieObject = piBatchSceneAnnotation(ieObject)
%
% Description:
%    Annotate/modify the provided scene object.
%
% Inputs:
%    ieObject - Object. A scene object.
%
% Outputs:
%    ieObject - Object. The modified scene object.
%
...
[scenelabel, objectList] = ...
    instanceSeg(meshImage, label, objects, depthmap);
        ...
        for kk = 1:length(uniqueList)
            ...
            for ll = 1:length(tmp_index)
                objectList.(fds{hh}){tmp_index(ll)}.position = ...
                    objectList.(fds{hh}){tmp_index(ll)}.position(:, 1);
                objectList.(fds{hh}){tmp_index(ll)}.rotate = ...
                    objectList.(fds{hh}){tmp_index(ll)}.rotate(:, 1:3);
            end
        end
    end
end
...
function [occluded, occludedRate, truncated, bbox2d, ignore] = ...
    getBBox(scene_mesh, index, depthmap)
% Retrieve a 2D bbox
%
% Syntax:
%   [occluded, occludedRate, truncated, bbox2d, ignore] = ...
%       getBBox(scene_mesh, index, offset)
%
% Description:
%    Retrieve a 2D representation of the BBOX as well as whether or not
%    there are occlusions (and the occlusion rate) or truncations.
%
% Inputs:
%    scene_mesh   - Matrix. A representation of the scene mesh image.
%    index        - Numeric. The index to begin at.
%    depthmap     - Matrix. The depth map in matrix form.
%
% Outputs:
%    occluded     - Boolean. A boolean for the presence of occlusion.
%    occludedRate - Numeric. The rate at which occlusion occurs.
%    truncated    - Boolean. A boolean for the presence of truncation.
%    bbox2d       - Struct. A structure containing the 2D BBox.
%    ignore       - Boolean. A boolean for whether or not to actually
...
posDepth = [bbox2d.xmin, bbox2d.ymin, bbox2d.xmax - bbox2d.xmin, ...
    bbox2d.ymax - bbox2d.ymin];
depthCrop = imcrop(depthmap, posDepth);
% check how far is the car
% depthCrop_tmp = imcrop(depthmap, [posDepth(1) - 20, posDepth(2), ...
%    posDepth(3) + 40, posDepth(4)]);
% Check mesh_image
...
function [scenelabel, objectList] = instanceSeg(scene_mesh, label, ...
    objects, depthmap)
% Create class and instance label map for training, and colorize it
%
% Syntax:
%   [sceneLabel, objectList] = instanceSeg(scene_mesh, label, ...
%       objects, depthMap)
%
% Description:
%    Create the class and instance label map for training, and then
%    colorize them for visualization.
%
% Inputs:
%    scene_mesh - Matrix. The scene mesh in matrix form.
%    label      - String. The file path for generated labels.
%    objects    - Object. The set of objects to add.
%    depthmap   - Matrix. The depth map in matrix form.
%
% Outputs:
%    scenelabel - Object. The scene object.
%    objectList - Object. The objects.
%
% Optional key/value pairs:
%    None.
%
...
for ii = 1:size(s, 1)
    ...
    if piContains(lower(s{ii}{2}), 'car')
        ...
        if ~isempty(C_1(scene_mesh == a))
            ...
            [objectList.car{count_car}.occluded, ...
                ...
                objectList.car{count_car}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
    ...
    if piContains(lower(s{ii}{2}), 'truck')
        ...
        if ~isempty(C_1(scene_mesh == a))
            ...
            [objectList.truck{count_truck}.occluded, ...
                ...
                objectList.truck{count_truck}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
            for jj = 1: length(objects)
                if strcmp(objects(jj).name, s{ii}{2}), break; end
            end
    ...
    if piContains(lower(s{ii}{2}), 'bus')
        ...
        if ~isempty(C_1(scene_mesh == a))
            ...
            [objectList.bus{count_bus}.occluded, ...
                ...
                objectList.bus{count_bus}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
            for jj = 1: length(objects)
                if strcmp(objects(jj).name, s{ii}{2}), break; end
            end
    ...
    if piContains(lower(s{ii}{2}), '_rider') || ...
            piContains(lower(s{ii}{2}), '_people')
        ...
        if ~isempty(C_1(scene_mesh == a))
            ...
            [objectList.rider{count_rider}.occluded, ...
                ...
                objectList.rider{count_rider}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
            for jj = 1: length(objects)
                if strcmp(objects(jj).name, s{ii}{2}), break; end
            end
    ...
    if piContains(lower(s{ii}{2}), 'bicycle') || ...
            ...
            ~piContains(lower(s{ii}{2}), '_people') && ...
            ~piContains(lower(s{ii}{2}), '_rider'))
        ...
        if ~isempty(C_1(scene_mesh == a))
            ...
            [objectList.bicycle{count_bicycle}.occluded, ...
                ...
                objectList.bicycle{count_bicycle}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
            for jj = 1: length(objects)
                if strcmp(objects(jj).name, s{ii}{2}), break; end
            end
    ...
    if piContains(lower(s{ii}{2}), 'motor')
        ...
        if ~isempty(C_1(scene_mesh == a))
            ...
            [objectList.motor{count_motor}.occluded, ...
                ...
                objectList.motor{count_motor}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
            for jj = 1:length(objects)
                if strcmp(objects(jj).name, s{ii}{2}), break; end
            end
...
%             disp(a);
            [objectList.pedestrian{count_pedestrian}.occluded, ...
                ...
                objectList.pedestrian{count_pedestrian}.ignore] = ...
                getBBox(scene_mesh, a, depthmap);
            for jj = 1: length(objects)
                if strcmp(objects(jj).name, s{ii}{2}), break; end
            end
...
% classVisulization = ...
%    fullfile(sceneFigureDir, [sceneName, '_class_color.png']);
% imwrite(uint8(ClassColorMap), classVisulization);
% instancelabel = ...
%    fullfile(sceneFigureDir, [sceneName, '_instance_label.png']);
% imwrite(uint16(InstanceMap), instancelabel);
% instanceColor = ...
%    fullfile(sceneFigureDir, [sceneName, '_instance_color.png']);
