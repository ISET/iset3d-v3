...
%
% Description:
%    Assign properties from an asset's recipe object on Flywheel to an
%    asset structure (asset).
%
% Inputs:
%    assetRecipe - Object. The asset recipe object from Flywheel.
%
% Outputs:
%   asset        - Struct. A structure capturing the information about the
%                  asset from the provided Flywheel recipe.
%
% Optional key/value pairs:
%    label       - String. The asset's label. Default ''.
%

% History:
%    XX/XX/XX  ZL   Created: Zhenyi Liu
...
for ii = 1: length(assetRecipe)
    ...
    for kk = 1:length(mlist)
        if  piContains(mlist{kk}, 'paint_base') && ...
                ~piContains(mlist{kk}, 'paint_mirror')
            name = mlist{kk};
            % A string labeling the material
            ...
            piMaterialAssign(thisR, material.name, target, ...
                'colorkd', colorkd);
        elseif piContains(mlist{kk}, 'carpaint') && ...
                ~piContains(mlist{kk}, 'paint_base')
            ...
            piMaterialAssign(thisR, material.name, target, ...
                'colorkd', colorkd);
        elseif piContains(mlist(kk), 'lightsback') || ...
                piContains(mlist(kk), 'lightback')
            name = cell2mat(mlist(kk));
            material = thisR.materials.list.(name);
        ...
        % empty the slot if there is a texture assigned
        elseif piContains(mlist(kk), 'tire')
            name = cell2mat(mlist(kk));
    ...
    for jj = 1:length(geometry)
        if ~isequal(lower(geometry(jj).name), 'camera') && ...
            ...
            % assign name on 'flywheel_label' to geometry(jj).name
            geometry(jj).name = sprintf('%s_%s', label, scenename);
            break;
        end
    end
    ...
    if ~isequal(assetRecipe{ii}.count, 1)
        for hh = 1: length(asset(ii).geometry)
            ...
            asset(ii).geometry(hh).position = ...
                repmat(pos, 1, uint8(assetRecipe{ii}.count));
            asset(ii).geometry(hh).rotate = ...
                repmat(rot, 1, uint8(assetRecipe{ii}.count));
        end
    end
