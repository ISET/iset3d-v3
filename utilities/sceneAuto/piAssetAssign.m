...
for ii = 1: length(assetRecipe)
    ...
    for kk = 1:length(mlist)
        if  strfind(mlist{kk},'paint_base') && ~strfind(mlist{kk},'paint_mirror')
            name = mlist{kk};
        ...
        elseif strfind(mlist{kk},'carpaint') && ~strfind(mlist{kk},'paint_base')
            name = cell2mat(mlist(kk));
            material = thisR.materials.list.(name);    % A string labeling the material
            target = thisR.materials.lib.carpaintmix;  %
            colorkd = piColorPick('random');
            piMaterialAssign(thisR,material.name,target,'colorkd',colorkd);
        elseif strfind(mlist(kk),'lightsback') || strfind(mlist(kk),'lightback')
            name = cell2mat(mlist(kk));
        ...
        elseif strfind(mlist(kk),'tire') % empty the slot if there is a texture assigned
            name = cell2mat(mlist(kk));
    ...
    for jj = 1:length(geometry)
        if ~isequal(lower(geometry(jj).name),'camera') && ...
                ~strfind(lower(geometry(jj).name),'light') && ...
                ~strfind(lower(geometry(jj).name),'rider_bike')
