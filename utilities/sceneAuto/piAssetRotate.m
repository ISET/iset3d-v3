...
%
% Syntax:
%   object = piAssetRotate(object, [varargin])
%
% Description:
%    Rotate an asset. Please note that only rotation around the y axis is
%    currently allowed.
%
% Inputs:
...
for dd = 1:instN
    for ii = 1:length(object)
        ...
        if isfield(object(ii), 'children')
            if length(object(ii).children) >= 1
                ...
                %         object(ii).size.pmin, object_position, -degree);
                %     object(ii).size.pmax = piPointRotate( ...
                %         object(ii).size.pmax, object_position, -degree);
            end
        end
    end
    % rotate lights
    ...
    %           %   plot([object_position(1) light(1)], ...
    %           %       [object_position(2) light(2)]);
    %           %   hold on
    %           %   axis([-15 15 -15 15]);
    %           position = piPointRotate(light, object_position, -degree);
    %           %   plot([object_position(1) position(1)], ...
    %           %       [object_position(2) position(2)]);
    %           object(jj).position = ...
    %               [position(1); object(jj).position(2); position(2)];
    %       end
    %   end
end
