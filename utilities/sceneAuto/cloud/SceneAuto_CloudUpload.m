...
% piAssetsDownload_cloud('session', 'others', ...
%     'acquisition', 'streetlight_short_002');
% piAssetsDownload_cloud('session', 'others', ...
...
for jj = 1:length(roadnamelist)
    ...
    % resourcesCombine(roadname, cityname, 52);
    % toc
    tic
    roadname = 'highway_straight_4lanes_001';
    resourcesCombineVehiclesOnly(roadname);
    toc
    ...
    allFiles = cell2mat(strcat({allFiles(cellfun(@(x) x(1) ~= ...
        '.', {allFiles(:).name})).name}, {' '}));

    % Remember where you are, and then change to the scene folder

    % Zip recursively but excluding certain file types and any other zip
    % files that might have been put here.
    fprintf('Zipping into %s\n', zipFileName);
    cmd = sprintf('zip -r %s %s -x *.jpg renderings/* *.zip *.json', ...
    ...
    cmd = sprintf('gsutil cp %s %s/', zipFileFullPath, cloudFolder);
    system(cmd);
    disp('ALL DONE!!!');
    toc
end
