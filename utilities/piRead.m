...
%       Integrator(V3), Renderer, LookAt, Transform, ConcatTransform, Scale
%
%    We modify the recipe programmatically to generate multiple renderings.
%
...
%
% Inputs:
%    fname         - String. a full path to a pbrt scene file
%
% Outputs:
%   recipe         - Object. A recipe object with the parameters needed to
%                    write a new pbrt scene file.
%
% Optional key/value pairs:
%    version       - Numeric. The PBRT version, 2 or 3. Default is 3.
%    readMaterials - Boolean. When a PBRT scene file is exported by
%                    cinema4d, the exporterflag is set and we read the
%                    materials file. If you do not want to read that file,
%                    set this to false.
%
% Notes:
%    * Caution:  The reading algorithm assumes that the following criteria
%      will be met for all PBRT files. piRead will not work otherwise.
%         - There is a block of text before WorldBegin and none after.
%         - Comments (indicated by '#' in the first character) and blank
%           lines are ignored.
...
% Examples:
%{
    fname = fullfile(piRootPath, 'data', 'V3', 'teapot', ...
...
% We could add - p.addParameter('outputFile') some day and over-ride the
% code below.
...
% [Note: BW - I don't understand why the spaces or tabs at the beginning of
% the line are not returned here.]
% fprintf('Reading\n');
...
if piContains(headerCheck_scene{1}, ...
...
cameraStruct = piBlockExtract(txtLines, 'blockName', 'Camera', ...
    'exporterFlag', exporterFlag);
...
samplerStruct = piBlockExtract(txtLines, 'blockName', 'Sampler', ...
    'exporterFlag', exporterFlag);
...
%% Extract film block
filmStruct = piBlockExtract(txtLines, 'blockName', 'Film', ...
    'exporterFlag', exporterFlag);
...
pfStruct = piBlockExtract(txtLines, 'blockName', 'PixelFilter', ...
    'exporterFlag', exporterFlag);
if isempty(pfStruct)
...
if ver == 2
    sfStruct = piBlockExtract(txtLines, ...
        'blockName', 'SurfaceIntegrator', 'exporterFlag', exporterFlag);
elseif ver == 3
    sfStruct = piBlockExtract(txtLines, ...
        'blockName', 'Integrator', 'exporterFlag', exporterFlag);
end

if isempty(sfStruct)
    warning(strcat('Cannot find "integrator" in PBRT file. Did you ', ...
        'forget to turn on the v3 flag?'));
...
if ver == 2
    rendererStruct = piBlockExtract(txtLines, 'blockName', 'Renderer', ...
        'exporterFlag', exporterFlag);
...
[~, transformBlock] = piBlockExtract(txtLines, ...
    'blockName', 'Transform', 'exporterFlag', exporterFlag);
if ~isempty(transformBlock)
    values = textscan(transformBlock{1}, ...
...
if isempty(transformBlock) && isempty(lookAtBlock)
    warning(strcat('Cannot find "LookAt" or "Transform" in PBRT ', ...
        'file. Returning default.'));
end

% If there's a concat transform, we use it to update the current camera
% position.
[~, concatTBlock] = piBlockExtract(txtLines, ...
    'blockName', 'ConcatTransform', 'exporterFlag', exporterFlag);
if ~isempty(concatTBlock)
    ...
    [from, to, up, flip] = ...
        piTransform2LookAt(lookAtTransform * concatTransform);
end
...
if exporterFlag
    if readmaterials
        ...
        % Convert all jpg textures to png format, only *.png & *.exr are
        % supported in pbrt.
