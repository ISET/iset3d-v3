...
%   deletezip - delete after unzipping (default false; only done if unzip is
%               true)
%
% Return
...
%
% Description
%   The PBRT data are stored as zip files that include the pbrt scene file along
%   with all of the necessary additional files.  These are typically stored
%   inside of folders (geometry, brdfs, spds, textures).  This function pulls
%   down the zip file containing everything and, by default, unzips the file so
%   that the scene and auxiliary files are in a single directory.
% 
...
%{
 aName = 'whiteScene';
 [fnameZIP, artifact] = piFetchPBRT(aName);
 [p,n,e] = fileparts(fnameZIP); pbrtFile = fullfile(p,n,[n,'.pbrt']);
 thisR = piRead(pbrtFile);
 thisR.outputFile = pbrtFile = fullfile(p,n,[n,'.pbrt'])
