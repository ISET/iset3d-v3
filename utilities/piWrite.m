...
%
% Description:
%    The pbrt scene file and all of its resources files are written out in
%    a working directory that will be mounted by the docker container.
%
...
%    you can set overwriteresources to false. Similarly if you do not want
%    to overwrite the pbrt scene file, set overwritepbrtfile to false.
%
% Inputs:
%    renderRecipe       - Recipe. A recipe object describing the rendering
%                         parameters. This includes the inputFile and the
%                         outputFile, which are used to find the
%                         directories containing all of the pbrt scene data
%
% Outputs:
%    workingDir         - String. path to the output directory mounted by
%                         the Docker container.
%
% Optional key/value pairs:
%    overwritePbrtFile  - Boolean. Whether or not to overwrite an existing
%                         PBRT scene file. Default true.
%    overwriteResources - Boolean. Whether or not to overwrite an existing
%                         resources file. Default true.
%    overwriteLensFile  - Boolean. Whether or not to overwrite an existing
%                         lens file. Default true.
%    overwriteMaterials - Boolean. Whether or not to overwrite an existing
%                         materials file. Default true.
%    overwriteGeometry  - Boolean. Whether or not to overwrite an existing
%                         geometry file. Default true.
%    createMaterials    - Boolean. Whether or not to create a new material.
%                         Default false.
%    lightsFlag         - Boolean. Whether or not to control the lighting
%                         in geometry.pbrt. Default false.
%    thisTrafficFlow    - Object. A trafficFlow object. Default [].
%
...
% This is a full directory copy, so all of the resources in the input
% directory are written to the docker working directory. In some cases we
% are looping so we can turn off the repeated copies with this flag.
if overwriteresources
    status = copyfile(inputDir, workingDir);
    if(~status)
        error(strcat("Failed to copy the input directory to docker's ", ...
            "working directory."));
    else
        fprintf('Copied the contents from: %s\nto: %s\n\n', ...
            inputDir, workingDir);
    end
end
...
if isequal(renderRecipe.get('optics type'), 'lens')
    ...
    elseif isfield(renderRecipe.camera, 'specfile')
        if (renderRecipe.version == 3)
            warning('Use lensfile, not specfile, for versions 3+');
        end
...
%% Write header
fprintf(fileID, ...
    '# PBRT file created with piWrite on %i/%i/%i %i:%i:%0.2f \n', clock);
...
fprintf(fileID, ...
    ...
    [renderRecipe.lookAt.from, renderRecipe.lookAt.to, ...
    renderRecipe.lookAt.up]);
fprintf(fileID, '\n');
...
for ofns = outerFields'
    ...
    if(~isempty(innerFields))
        for ifns = innerFields'
            ifn = ifns{1};
            % [Note: BW - Skip these since we've written these out earlier,
            % they are localized to pbrt2ISET but not pbrt scene files. We
            % make a second copy of the lens file in the workingDir because
            % some docker containers require it. In the future, we should
            % exclude lens file and specfile.]
            if(strcmp(ifn, 'type') || strcmp(ifn, 'subtype') || ...
                    strcmp(ifn, 'subpixels_h') || ...
                    strcmp(ifn, 'subpixels_w') || ...
                    strcmp(ifn, 'motion') || strcmp(ifn, 'subpixels_w'))
                continue;
            end
            ...
            if(strcmp(currType, 'string') || ischar(currValue))
                ...
                % [Note: BW - Perhaps we should have a better test here,
                % say an exist() test.]
                [~, name, ext] = fileparts(currValue);

                if(~isempty(ext))
                    % OK, it has an extension. So we swing into action.
                    % First, copy the file to the working directory -
                    % unless it is a lens file, in which case it is already
                    % in place (see above)
                    fileName = strcat(name, ext);
                    if ~(strcmp(ifn, 'specfile') || ...
                            strcmp(ifn, 'lensfile'))
                        [success, ~, id] = copyfile(currValue, workingDir);
                        if ~success && ~strcmp(id, ...
                                'MATLAB:COPYFILE:SourceAndDestinationSame')
                            warning('Problem copying %s\n', currValue);
                        end
                        % Update the file for the relative path
                        currValue = fileName;
                    else
                        % It is a lens, so just update the name. It was
                        % already copied
                        if ispc()
                            currValue = strcat('lens/', strcat(name, ext));
                        else
                            currValue = fullfile('lens', ...
                                strcat(name, ext));
                        end
                    end
                end
            ...
            elseif(strcmp(currType, 'spectrum') && ~ischar(currValue))
                % A spectrum of type [wave1 wave2 value1 value2].
                % TODO: There are probably more variations of this...
