...
% Angle at which the spotlight intensity begins to fall off at the edges.
...
if idxL
    ...
    if find(piContains(varargin, 'lightspectrum'))
        lightSpectrum = p.Results.lightspectrum;
    else
        [~, lightSpectrum] = fileparts(lightsource{idxL}.spectrum);
    end
    ...
    if find(piContains(varargin, 'coneAngle'))
        coneAngle = p.Results.coneangle;
    else
        coneAngle = lightsource{idxL}.coneangle;
    end

    if find(piContains(varargin, 'conDeltaAngle'))
        conDeltaAngle = p.Results.conedeltaangle;
    else
        conDeltaAngle = lightsource{idxL}.conedeltaangle;
    end
...
if ischar(lightSpectrum)
    ...
    for ii = 1: length(thisLight.data)
        fprintf(fid, '%d %d \n', thisLight.wavelength(ii), ...
            thisLight.data(ii) * spectrumScale);
    end
    fclose(fid);
else
    % to do
...
switch type
    case 'point'
        ...
        if p.Results.cameracoordinate
            ...
            lightSources{1}.line{3, :} = sprintf(['LightSource ', ...
                '"point" "spectrum I" "spds/lights/%s.spd"'], ...
                lightSpectrum);
            lightSources{1}.line{end+1} = 'AttributeEnd';
        else
            lightSources{1}.line{1, :} = sprintf(['LightSource ', ...
                '"point" "spectrum I" "spds/lights/%s.spd" "point ', ...
                'from" [%d %d %d]'], lightSpectrum, from);
        end
    case 'spot'
        lightSources{1}.type = 'spot';
        lightSources{1}.line{1, :} = sprintf(['LightSource "spot" ', ...
            '"spectrum I" "spds/lights/%s.spd" "point from" ', ...
            '[%d %d %d] "point to" [%d %d %d]'], lightSpectrum, from, to);
        thisConeAngle = sprintf('float coneangle [%d]', coneAngle);
        thisConeDelta = ...
            sprintf('float conedelataangle [%d]', conDeltaAngle);
        lightSources{1}.line{2, :} = [lightSources{end + 1}.line{2}, ...
            thisConeAngle, thisConeDelta];
    case 'laser' % not supported for public
        lightSources{1}.type = 'laser';
        lightSources{1}.line{1, :} = sprintf(['LightSource "laser" ', ...
            '"spectrum I" "spds/lights/%s.spd" "point from" ', ...
            '[%d %d %d] "point to" [%d %d %d]'], lightSpectrum, from, to);
        thisConeAngle = sprintf('float coneangle [%d]', coneAngle);
        thisConeDelta = ...
            sprintf('float conedelataangle [%d]', conDeltaAngle);
        lightSources{1}.line{1, :} = [lightSources{end + 1}.line{2}, ...
            thisConeAngle, thisConeDelta];
    case 'distant'
