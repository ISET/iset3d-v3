...
% Create a white response at the sensor to color calibrate
%
% Syntax:
%   [correctionMatrix, sensor, oi] = piWhiteField(lfRecipe, [varargin])
%
% Description:
%    The white field response (correctionMatrix) is computed. This matrix
%    can be used to normalize the sensor volts from a light field camera,
%    accounting for the inhomogeneity across the subpixels and for color
%    sensitivity differences. When the correction matrix is applied, a
%    large white field will be rendered as white.
%
%    The white scene is calculated using the same camera parameters as in
%    the original light field. This correctionMatrix can be reused, but it
%    must be recalibrated if you change the number of pinholes, subpixel
%    properties, lens, or film.
%
%    By default, the white scene is treated as if it is a 10 lux image.
%    The sensor created here is the one returned from:
%       sensorCreate('light field', oi).
%
% Inputs:
%    lfRecipe           - Recipe. Recipe describing the light field camera
%                         you used to render.
%
% Outputs:
%    correctionMatrix   - Matrix. White scene's sensor pixel voltage array
%    sensor             - Sensor. Contains the responses to the white scene
%    oi                 - Optical Image. The light field of the white scene
%
% Optional key/value pairs:
%    'mean illuminance' - Numeric. Set oi mean illuminance (default 10 lux)
%
% See Also:
%   s_piReadRenderLF, ip2lightfield
%
% Notes:
...
%    - We need a way to just save and load the white calibration values
%      rather than re-running every time.
%
...
% Examples:
%{
    % Based on running the chess set scene. Neaten or eliminate soon.
    % Why are the chess voltages are so low? Probably exposure duration?
...
whiteRecipe.set('outputFile', ...
    fullfile(piRootPath, 'local', 'whiteScene.pbrt'));
piWrite(whiteRecipe);
