...
% If we are using the realistic eye, we skip this step since the lens file
% is unique.
...
try
    ...
    if isfield(recipe.camera, 'aperturediameter') || ...
            isfield(recipe.camera, 'aperture_diameter')
        ...
        while continueRead && dStart <= length(firstColumn)
            ...
            if ~isempty(compare{1}), continueRead = false; end
            dStart = dStart + 1;
        end
