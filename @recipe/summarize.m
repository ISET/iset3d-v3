...
%
% Description:
%   Prints a quick summary of the critical rendering recipe parameters, for
%   several of the cases, the object described is returned.
%
% Inputs:
%   str      - String. The string representing the summary of the relevant
%              recipe parameter(s). Options are: all, file, render, camera,
%              film, lookat, assets, materials, and metadata.
%
% Outputs:
%   out      - Object. Either the object described our empty
%   namelist - Array. A string array that is either a sorted list of the
%              asset names (if str = 'assets'), or the name of the
%              materials (if str = 'materials').
%
...
validStr = {'all', 'file', 'render', 'camera', 'film', 'lookat', ...
    'assets', 'materials', 'metadata'};
p = inputParser;
...
switch str
    ...
    case 'file'
        ...
        if isfield(thisR, 'exporter')
            fprintf('Exported by %s\n', thisR.exporter);
        end
        fprintf('\n');
    ...
    case 'assets'
        ...
        fprintf('Static  assets: %d\n', nAssets - nMoving);
        namelist = cell(nAssets, 1);
        for ii = 1:nAssets, namelist{ii} = thisR.assets(ii).name; end
    ...
    case 'materials'
        ...
        fprintf('Number:\t%d\n', numel(namelist));
        [~, filename, ext] = ...
            fileparts(thisR.materials.inputFile_materials);
        fprintf('File:\t%s\n', [filename, ext])
        fprintf('\n');
